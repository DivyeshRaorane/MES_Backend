...
export async function validateBobbinQC(bobbinNo) {
  ...
  try {
    ...
    // --- STEP 1: Look for missing bottom data and copy from top ---
    ...
    for (const pair of topBottomPairs) {
      ...
      // If bottom value is missing/null, but top value exists, borrow top value
      if ((bottomVal === null || bottomVal === undefined || bottomVal === '') && 
          (topVal !== null && topVal !== undefined && topVal !== '')) {
        measurement[pair.bottom] = topVal; 
        synchronizedPairsToUpdate.push(pair);
      }
    }


    // --- STEP 2: Proceed to grading (no null check — handled by grade engine) ---
    ...
    console.log("Sync:", parametersToCheck)
